 ...
 */
export const encryptWithWalletPublicKey = async (data: string, publicKey: string): Promise<string> => {
  // Get consistent encryption parameters
  const { iv, ephemeralKey } = getEncryptionParameters()

  // For now, return a mock encrypted value
  // In production, you would use eccrypto or similar library
  const mockEncrypted = Buffer.from(`${data}_encrypted_with_${publicKey}`).toString("hex")
 ...
 */
export function formatVanaFileId(url: string, timestamp = Date.now()): string {
  return `vana_submission_${timestamp}_${url.substring(url.lastIndexOf("/") + 1)}`
}
