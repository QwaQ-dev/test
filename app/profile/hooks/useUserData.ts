...
// Updated types to match the Google API
  ...
  useEffect(() => {
    ...
    const fetchUserData = async () => {
      ...
      try {
        ...
        if (userInfoResponse.status === 401 || driveInfoResponse.status === 401) {
        ...
        if (!driveInfoResponse.ok) {
          const errorData = await driveInfoResponse.json().catch(() => ({ error: "Unknown error" }))
          console.error("Drive info error:", errorData)
          setError("Failed to fetch drive information. Please try again later.")
