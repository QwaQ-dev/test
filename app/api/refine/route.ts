...
export async function POST(request: Request) {
  try {
    ...
    if (!refinementEndpoint) {
      return NextResponse.json({ error: "Refinement endpoint not configured" }, { status: 500 })
    }

    if (!fileId || !encryptionKey) {
      return NextResponse.json({ error: "Missing required parameters: file_id or encryption_key" }, { status: 400 })
    }
    ...
    return NextResponse.json({ error: "Failed to process refinement request" }, { status: 500 })
  }
}
