...
// Mock function - you'll need to implement this based on your chain
const getTransactionUrl = (hash: string) => `https://etherscan.io/tx/${hash}`
    ...
    <div className="space-y-4">
      ...
      <div className="space-y-3 bg-slate-50 p-4 rounded-md text-sm">
        ...
        <div className="grid grid-cols-2 gap-2">
          ...
          <div className="font-mono text-xs truncate">{contributionData.fileId || "Processing..."}</div>
          <div className="text-muted-foreground">Transaction Hash</div>
          <div className="font-mono text-xs truncate flex items-center">
            <span className="truncate">{contributionData.transactionReceipt?.hash || "Pending..."}</span>
          ...
          {proofRequested && (
            <>
              ...
              <div className="font-mono text-xs truncate">{contributionData.teeJobId || "Processing..."}</div>
            </>
          )}
          {fullyCompleted && (
            <>
              ...
              <div className="font-mono text-xs truncate flex items-center">
                <span className="truncate">{contributionData.rewardTxHash || "Pending..."}</span>
      ...
      {proofCompleted && contributionData.teeProofData && (
        <div className="space-y-3 bg-slate-50 p-4 rounded-md text-sm">
          ...
          <div className="max-h-48 overflow-y-auto bg-slate-100 p-2 rounded-md font-mono text-xs">
            <pre className="whitespace-pre-wrap">{JSON.stringify(contributionData.teeProofData, null, 2)}</pre>
          </div>
        </div>
      )}
      ...
      {userInfo && <ContributionSummary userInfo={userInfo} driveInfo={driveInfo} isEncrypted={true} />}
    </div>
  )
}
