...
// Mock contract instances - you'll need to implement these
const DataRegistry = () => ({
...
export function useAddFile() {
  ...
  const { address: dataLiquidityPoolAddress } = Controller("DataLiquidityPoolProxy")

  /**
   * Add file to blockchain and wait for receipt
   */
  const addFile = async (fileUrl: string, encryptionKey: string): Promise<TransactionReceipt | null> => {
    setIsAdding(true)
    setError(null)
    ...
    try {
      ...
      console.error(err)
      const error = err instanceof Error ? err : new Error("Failed to add file")
      setError(error)
      ...
      if (err instanceof Error) {
        ...
        const errorMessage = errorObj.reason || errorObj.message || JSON.stringify(err)
        setContractError(errorMessage)
      } else {
